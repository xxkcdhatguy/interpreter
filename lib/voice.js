...
const LANG_IDS = {
  English: "en",
  Portuguese: "pt-BR", // Brazilian, not European
  Spanish: "es",
  French: "fr",
  German: "de-DE",
...
/**
 * The /voices/clone endpoint rejects locales ("pt-BR", "de-AT") and accepts
 * only bare language codes, unlike /tts/bytes which uses the locale to steer
 * the accent. Strip the region for cloning.
 */
...
async function cloneVoice(audioBuffer, filename, sourceLangCode, speakerName) {
  const form = new FormData();
  form.append("clip", new Blob([audioBuffer]), filename || "clip.webm");
  // Name the voice after the speaker so it is identifiable later; these are
  // kept, not discarded, so "turn-1788…" would be meaningless in the dashboard.
  const stamp = new Date().toISOString().slice(0, 10);
  form.append("name", `${(speakerName || "Speaker").slice(0, 40)} — ${stamp}`);
  form.append(
    "description",
    `Cloned by the Interpreter app on ${stamp}${
      speakerName ? ` for ${speakerName}` : ""
    }`
  );
  ...
  form.append("language", baseLanguage(sourceLangCode) || "en");
